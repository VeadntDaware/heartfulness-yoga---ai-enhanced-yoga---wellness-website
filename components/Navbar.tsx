...
const NavLink: React.FC<{ to: string; children: React.ReactNode; active?: boolean }> = ({ to, children, active }) => (
  <Link
    to={to}
    className={`font-medium text-sm tracking-wide transition-colors duration-200 ${active ? 'text-teal-900 font-bold border-b-2 border-teal-300' : 'text-gray-600 hover:text-black'
      }`}
  >
    {children}
  </Link>
    ...
    <nav className="fixed top-0 w-full bg-white/90 backdrop-blur-md z-50 border-b border-gray-100 h-20 flex items-center">
      <div className="max-w-7xl mx-auto px-6 w-full flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <NavLink to="/find-trainer" active={location.pathname === '/find-trainer'}>Find Trainer</NavLink>
          <NavLink to="/teach" active={location.pathname === '/teach'}>Teach</NavLink>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-20 left-0 w-full bg-white border-b border-gray-100 p-6 flex flex-col gap-4 shadow-xl z-50">
          ...
          <Link to="/find-trainer" onClick={() => setMobileMenuOpen(false)} className="text-lg font-medium">Find Trainer</Link>
          <Link to="/teach" onClick={() => setMobileMenuOpen(false)} className="text-lg font-medium">Teach</Link>
